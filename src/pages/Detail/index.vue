                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        <div class="chooseArea">
                            ...
                            <dl>
                                <dt class="title" changeprice="0">颜色</dt>
                                <dd>金色</dd>
                                <dd>蓝色</dd>
                                <dd>粉色</dd>
                            </dl>
                        ...
                        <div class="cartWrap">
                            <div class="controls">
                                <!-- @change=""事件与blur事件相比，无法在失去焦点时判断，change事件自带失去焦点事件，
                                当失去焦点时，如果值发生了改变，才会执行回调，否则即便失去焦点，也不会执行回调 -->
                                <input
                                    autocomplete="off"
                                    class="itxt"
                                    v-model="skuNum"
                                    @change="skuNum > 1 ? skuNum : (skuNum = 1)"
                                />
                                <a
                                    href="javascript:"
                                    class="plus"
                            ...
                            <div class="add">
                                <a href="javascript:" @click="addShopCart()"
                                    >加入购物车</a
                                >
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        // async函数返回promise对象，它的成功与失败以及结果要看return
        // 如果return返回的不是一个Promise对象，那么返回结果就是成功的，且值，就是return的值
        // 如果return返回的是一个promise对象
        //          如果这个promise是成功，那么结果就是return的结果
        //          如果这个promise是失败的，那么结果就是失败的reason
        // 另外，如果没有return，而是throw的时候，结果也是失败的promise对象，原因就是抛出错误的原因
        async addShopCart() {
            try {
                // await返回的数据如果不是promise对象，此值直接作为await的返回值，如果是成功的promise对象，
                // 将返回成功的promise对象，如果是失败的promise对象，需要使用try。。。catch捕获处理
